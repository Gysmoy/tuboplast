...
import SucursalesRest from '../Actions/Admin/SucursalesRest.js'
import { getDepartments, getDistricts, getProvinces } from '../Utils/ubigeo.js'
import { loadGoogleMapsApi } from '../Utils/googleMaps.js'

const sucursalesRest = new SucursalesRest()
const DEFAULT_CENTER = { lat: -12.046374, lng: -77.042793 }

const PLACE_CSS = `
.wfb-act{width:32px;height:32px;border-radius:8px;display:inline-flex;align-items:center;justify-content:center;border:0;font-size:13px;transition:filter .15s;}
.wfb-act:hover{filter:brightness(.95);}
.wfb-act.edit{background:#e8f0ff;color:#3b82f6;}
.wfb-act.del{background:#fcebeb;color:#e24b4a;}
.wfb-chip{display:inline-flex;align-items:center;padding:3px 10px;border-radius:50rem;font-size:11px;font-weight:600;background:#e6effa;color:#004991;}
.wfb-btn{height:40px;padding:0 14px;border-radius:12px;background:#004991;color:#fff;font-weight:600;font-size:13px;border:0;display:inline-flex;align-items:center;gap:6px;transition:background .2s;}
.wfb-btn:hover{background:#003b7a;color:#fff;}
.wfb-btn.foot{height:38px;border-radius:10px;}
.wfb-btn:disabled{opacity:.65;cursor:default;}
.wfb-btn.outline{background:#fff;border:1px solid #dce5f0;color:#5b6577;}.wfb-btn.outline:hover{background:#f4f8fd;color:#0f2540;}
.wfb-h2{font-size:18px;font-weight:700;line-height:1.25;margin:0;color:#0f2540;}
.wfb-sec{border:1px solid #eef2f8;border-radius:12px;padding:16px;}
.wfb-sec h4{font-size:14px;font-weight:700;color:#0f2540;margin:0 0 14px;display:flex;align-items:center;}
.wfb-modal-ovl{position:fixed;inset:0;z-index:1100;background:rgba(15,23,42,.5);display:flex;align-items:flex-start;justify-content:center;padding:3vh 12px;}
.wfb-modal{position:relative;width:min(960px,96vw);max-height:94vh;background:#fff;border-radius:16px;box-shadow:0 24px 60px rgba(15,37,64,.25);display:flex;flex-direction:column;overflow:hidden;}
.wfb-modal form{display:flex;flex-direction:column;min-height:0;flex:1;}
.wfb-modal-head{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:16px 20px;border-bottom:1px solid #eef2f8;flex-shrink:0;}
.wfb-modal-body{overflow-y:auto;padding:16px 20px;display:flex;flex-direction:column;gap:14px;flex:1;}
.wfb-modal-foot{display:flex;justify-content:flex-end;gap:8px;padding:12px 20px;border-top:1px solid #eef2f8;flex-shrink:0;}
.wfb-close{width:36px;height:36px;border-radius:10px;border:0;background:none;color:#8a93a6;font-size:18px;}
.wfb-close:hover{background:#f4f8fd;color:#0f2540;}
.wfb-modal .form-label{font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.04em;color:#8a93a6;margin-bottom:3px;}
.wfb-err{display:flex;align-items:center;gap:8px;background:#fcebeb;color:#b42318;border-radius:10px;padding:10px 14px;font-size:13px;font-weight:500;}
`
...
const Branches = ({ gmapsApiKey }) => {
  ...
  useEffect(() => {
    sucursalesRest.ubigeoOptions().then((data) => {
  ...
  const performDelete = async () => {
    ...
    if (!row) return
    setDeleting(true)
    const ok = await sucursalesRest.delete(row.id)
    setDeleting(false)
    if (!ok) return
  ...
  const onSaveSubmit = async (e) => {
    ...
    setFormError('')
    setLoading(true)
    const result = await sucursalesRest.save({
      id: dataLoaded?.id,
      department,
      province,
      district,
  ...
  const columns = [
    { key: 'department', header: 'Departamento', field: 'department', nowrap: true, render: (d) => <span className='fw-semibold'>{d.department}</span> },
    ...
    { key: 'ubigeo', header: 'Ubigeo', field: 'ubigeo', width: 92, render: (d) => <span className='wfb-chip'>{d.ubigeo || '-'}</span> },
    { key: 'address', header: 'Dirección', field: 'address', render: (d) => <span style={{ color: '#5b6577' }}>{d.address || '-'}</span> },
    { key: 'reference', header: 'Referencia', field: 'reference', render: (d) => <span style={{ color: '#5b6577' }}>{d.reference || '-'}</span> },
    {
    ...
    },
    {
      ...
      render: (d) => (
        <div className='d-flex align-items-center justify-content-center gap-1'>
          <button className='wfb-act edit' title='Editar' onClick={() => onModalOpen(d)}><i className='mdi mdi-square-edit-outline'></i></button>
          <button className='wfb-act del' title='Eliminar' onClick={(e) => askDelete(d, e)}><i className='mdi mdi-trash-can'></i></button>
        </div>
      ),
    },
  ]
  ...
  return (
    <>
      ...
      <AdminTable
        ...
        countSuffix='sucursales'
        defaultSort={[{ selector: 'department', desc: false }]}
        minWidth={1100}
        headerActions={<button type='button' className='wfb-btn' onClick={() => onModalOpen(null)}><i className='mdi mdi-plus'></i> Nueva sucursal</button>}
        columns={columns}
      />

      <div className='wfb-modal-ovl' style={{ display: isModalOpen ? 'flex' : 'none' }} onMouseDown={closeForm}>
        <div className='wfb-modal' onMouseDown={(e) => e.stopPropagation()}>
          <form onSubmit={onSaveSubmit}>
            <div className='wfb-modal-head'>
              <h3 className='wfb-h2' style={{ fontSize: 16 }}>
                <i className={`mdi ${dataLoaded ? 'mdi-square-edit-outline' : 'mdi-plus-box'} me-1`} style={{ color: '#004991' }}></i>
                {dataLoaded ? 'Editar sucursal' : 'Nueva sucursal'}
              </h3>
              <button type='button' className='wfb-close' onClick={closeForm}><i className='mdi mdi-close'></i></button>
            </div>

            <div className='wfb-modal-body'>
              {formError && <div className='wfb-err'><i className='mdi mdi-alert-circle-outline'></i>{formError}</div>}

              <div className='wfb-sec'>
            ...
            <div className='wfb-modal-foot'>
              <button type='button' className='wfb-btn outline foot' onClick={closeForm} disabled={loading}>Cancelar</button>
              <button type='submit' className='wfb-btn foot' disabled={loading}>
                {loading
                  ? <><span className='spinner-border spinner-border-sm'></span> Guardando...</>
                  : <><i className='mdi mdi-content-save'></i> {dataLoaded ? 'Guardar cambios' : 'Crear sucursal'}</>}
              </button>
            </div>
          </form>
        </div>
      ...
      <ConfirmModal
        ...
        message={confirmTarget ? `Se eliminará la sucursal de ${confirmTarget.district || confirmTarget.address || `#${confirmTarget.id}`}. Esta acción no se puede deshacer.` : ''}
...
CreateReactScript((el, properties) => {
  createRoot(el).render(
    <Adminto {...properties} title='Sucursales'>
      <Branches {...properties} />
    </Adminto>
  )
})
