...
const Sucursales = ({ gmapsApiKey }) => {
  ...
  const onDeleteClicked = async (id) => {
    const ok = await sucursalesRest.delete(id)
    if (!ok) return
    refreshGrid()
  }
  ...
  return (
    <>
          ...
          },
          {
            ...
            cellTemplate: (container, { data }) => {
              ReactAppend(container,
                <SwitchFormGroup
                  id={`switch-branch-${data.id}`}
                  checked={Boolean(data.status)}
                  noMargin
                  onChange={async () => {
                    await sucursalesRest.status({ id: data.id, status: data.status })
                    refreshGrid()
                  }}
                />
              )
      ...
      <Modal
        modalRef={modalRef}
        title={dataLoaded ? 'Editar sucursal' : 'Nueva sucursal'}
        onClose={resetForm}
