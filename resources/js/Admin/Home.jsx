...
const Home = ({ dashboard, filter }) => {
  ...
  useEffect(() => {
    ...
    const options = {
      ...
      xaxis: {
        categories: d.chart.labels,
        title: { text: d.chart.x_title || 'Día del mes', style: { color: '#8a93a6', fontWeight: 600 } },
        tickPlacement: 'on',
        labels: { rotate: -45, rotateAlways: false, hideOverlappingLabels: true, style: { fontSize: '11px', colors: '#8a93a6' } },
        axisBorder: { show: false },
        axisTicks: { show: false },
      },
      yaxis: [
        { min: 0, forceNiceScale: true, tickAmount: 5, title: { text: 'Cantidad', style: { color: '#8a93a6', fontWeight: 600 } }, labels: { formatter: (v) => Math.round(v) } },
        { opposite: true, min: 0, title: { text: 'Monto (S/)', style: { color: '#8a93a6', fontWeight: 600 } }, labels: { formatter: (v) => `S/ ${Math.round(v).toLocaleString('es-PE')}` } },
      ],
      colors: ['#004991', '#e0a800'],
      fill: { opacity: [0.95, 1] },
      dataLabels: { enabled: false },
      grid: { borderColor: '#eef2f8', strokeDashArray: 4, padding: { left: 6, right: 6 } },
